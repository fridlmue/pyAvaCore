...
class TestBavariaAmPm(unittest.TestCase):
    ...
    def test_bavaria_ampm(self):
        ...
        self.assertIn('BYAMM', report.region)
        self.assertNotIn('BYBGD', report.region)
        self.assertEqual(report.dangerRating[0].mainValue, 'moderate')
        # self.assertEqual(report.dangerRating[0].valid_elevation, None)
        ...
        self.assertEqual(report.validTime.startTime.isoformat(), '2021-02-20T12:00:00')
        self.assertEqual(report.validTime.endTime.isoformat(), '2021-02-20T23:59:59')
        self.assertIn('BYBGD', report.region)
        self.assertNotIn('BYCHG', report.region)
        self.assertEqual(report.dangerRating[0].mainValue, 'moderate')
        #self.assertEqual(report.dangerRating[0].valid_elevation, '>2000')
        self.assertEqual(report.dangerRating[0].elevation.lowerBound, '2000')
