...
class TestNorway(unittest.TestCase):
    ...
    def test_norway(self):
        with open(f'{__file__}.json') as fp:
            data = json.load(fp)
        varsom_report = data
        reports = get_reports_fromjson('NO-3016', varsom_report, fetch_time_dependant=False)
        ...
        self.assertEqual(report.avalancheProblems[0].problemType, 'persistent_weak_layers')
